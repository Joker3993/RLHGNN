...
def Final_test(args, choice, seed):
    print("start testing...")
    ...
    result_path = f"result_Pretrain/action_{choice}_{seed}/" + args.dataset
    ...
    path = f"Pretrain/action_{choice}_{seed}/" + args.dataset
    if not os.path.exists(path):
        os.makedirs(path, exist_ok=True)

    dataset_test = MyDataset(name=args.dataset + "_" + str(fold), type="test", choice=choice)
...
if __name__ == '__main__':
    ...
    for eventlog in tqdm(list_eventlog):
        for seed in seed_list:
            print(f"--------------预训练模型: {eventlog}, seed {seed} ------------")
            for choice in range(4):
                print(f"-------------- {eventlog} 动作{choice} seed {seed} ------------")
                ...
                Final_test(args, choice=choice, seed=seed)
